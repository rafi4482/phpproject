...
session_start();
 ...
 ?>
   ...
   <form class="myform" action="agentinvoice.php" method="post">
...
if(isset($_POST['submit'])){
    ...
    //echo "$username1";
    $bid=$_POST['bid'];
    $q="select * from booking where bid='$bid' and aid='$username1' and status='returned' ";
    ...
    if(mysqli_num_rows($qrun)>0){
     while($rm=mysqli_fetch_assoc($qrun)){
      ...
      <label><b>Comm-Rate(%)<font color=red> $crate1  </font></b></label> 
        ...
        echo '<script>alert("Booking NOT Found or car not returned ")</script>';
    }
...
?>
   ...
   if(isset($_POST['back1'])){
    echo "<script> location.href='agentboard.php'</script>" ;
   }
